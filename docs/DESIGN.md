...
- **worker 유휴 재지시.** 에이전트 턴이 프로바이더 API 오류로 중단되면
  ...
  이어가기 재지시를 보낸다(`nudges` 카운트 — 라운드/작업 전환 시 리셋).
  재지시가 소진되면 Stalled로 승격해 사람을 부른다(`krill resume`이 복구
  경로). **권한 프롬프트로 분류되면 절대 타이핑하지 않는다** — 대화상자에
  send-keys는 임의 버튼을 누르는 것과 같다(실측). 분류 불명도 타이핑하지
  않는다. 분류는 순수 함수, 프리셋(claude-code)의 페이로드 형태에만
  의존하고 다른 에이전트는 페이로드가 없어 자연히 제외.
- **일시 장애에는 지연 백오프 재시도 (anago M1 실측, BACKLOG #11).**
  529 Overloaded 같은 프로바이더 장애는 시간이 해결하는데, 즉시 재지시는
  같은 장애 창 안에서 또 죽고, 2차 idle Notification 재발화는 보장이
  없다(실측 3시간 손실). `duet-gate`의 detached 단명 자식 패턴을
  재사용한다: 재지시 후 `krill duet-nudge -i <id> -d <초>` 자식을 띄워
  백오프(1분→5분→15분) 간격으로 재확인하고, 여전히 유휴면 다음 재지시
  (최대 3회), 소진 시 Stalled + ntfy. worker가 다시 일하고 있으면(최근
  출력이나 Done/Reviewer 전환) 자식은 조용히 사라진다. 데몬 0 유지 —
  자식은 단명이고 상태의 원본은 여전히 `duet.kv`다.
- **긴 순회는 주기적으로 컨텍스트를 비운다 (BACKLOG #9).** 40+ 태스크
  순회에서 worker 세션이 컨텍스트 한계에 도달한다(실측 2회). 에이전트별
  압축 명령을 config 데이터로 둔다(`[agents.claude] compact = "/compact"`
  — 원칙 2: 벤더를 코드에 하드코딩하지 않는다). `[duet] compact_every =
  N`(기본 0 = 끔)이면 plan 순회에서 N개 태스크 완료마다 다음 태스크 지시
  전에 압축 명령을 한 줄 보낸다. 압축 명령이 없는 에이전트는 건너뛴다.
