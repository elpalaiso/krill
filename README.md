...
백오프(1분→5분→15분)로 자동 재지시하고, 그래도 멈추면 같은 방식으로
사람을 부릅니다. 긴 순회에는 `[duet] compact_every = 8`을 켜 두면 N개
작업마다 worker의 컨텍스트를 압축시킵니다(`[agents.*] compact` 명령이
설정된 에이전트만, 예: claude의 `/compact`).
