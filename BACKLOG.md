...
### 2. codex TUI가 리뷰 턴 후 자체 종료 → 리뷰어 사망 — ✅ 관찰 종결 (재현 안 됨)

**종결 (2026-08-19)**: 최초 1회 이후 재현되지 않았다 — M0 순회 재기동
후 30+ 리뷰 턴, M1 순회 전체(수십 턴, notify 로그로 확인)에서 같은
codex-cli 0.147.0 리뷰어가 계속 생존. 최초 사망은 세션 초기의 수동
send-keys 개입과 겹친 시점이라 외부 요인 가능성이 높다. 재발하더라도
#1의 `deliver_instruction` 재스폰이 안전망이므로 조사를 종료한다.
재발 시 이 항목을 다시 연다.
...
### 9. plan 순회에서 worker 세션 컨텍스트가 소진됨 — ✅ 수정됨

**수정 (2026-08-19)**: `[agents.*] compact`(에이전트별 압축 명령 — config
데이터, 예: claude `/compact`) + `[duet] compact_every = N`(기본 0 = 끔).
plan 순회에서 N개 작업 완료마다 다음 태스크 지시 전에 압축 명령을 보낸다.
압축 명령이 없는 에이전트는 건너뛴다. DESIGN §12.1 결정 7.
...
## 2026-08-19 — anago M1 순회 중

### 11. 일시적 프로바이더 장애에 지연 재시도가 없다 — ✅ 수정됨

**수정 (2026-08-19)**: 제안대로 구현 — `nudged` 플래그를 `nudges` 카운트
(최대 3회, 구파일 호환)로 확장하고, 재지시 후 `krill duet-nudge -i <id>
-d <초>` detached 자식이 백오프(1분→5분→15분)로 재확인·재지시한다.
worker가 다시 일하고 있으면(Active/Done) 자식은 조용히 사라지고, 소진
시 Stalled + ntfy. idle Notification 재발화 비보장 문제도 함께 해소.

anago M1 순회의 B1 태스크에서 Claude API **529 Overloaded**(서버측 일시
장애)가 worker 턴을 끊었다. M5d의 유휴 재지시(#8)는 정확히 발동했지만
**즉시 1회** 재시도라 같은 장애 창 안에서 다시 죽었고, 이후는 2차 idle
Notification 재발화에 의존하는데 이는 보장이 없어 Stalled 승격까지
**약 3시간**이 걸렸다. "시간이 해결하는 장애"에는 지연 재시도가 필요하다.

제안: `duet-gate`가 이미 쓰는 **detached 단명 자식 패턴**을 재사용 —
NudgeWorker 전달 후(또는 유휴 재발 시) `krill duet-nudge -i <id> --delay
<n>` 자식을 spawn해 n초 뒤 상태를 재확인하고 여전히 awaiting=worker +
유휴면 재지시. 백오프(예: 1분→5분→15분, 총 N회는 config)로 반복하고
소진 시 Stalled 승격. 데몬 0 유지(자식은 단명). 부수 효과: idle
Notification 재발화 비보장 문제도 자식이 자체 타이머를 가지므로 해소.
주의: 권한 프롬프트 분류면 여전히 절대 타이핑하지 않는다.

관찰(같은 사건): worker 컨텍스트 349.7k 도달 — #9 재현. 지연 재시도와
별개로 #9의 /compact 지시 옵션이 순회 후반 품질에 점점 중요해진다.
...
재지시), F4(#10 plan PR 본문 자동 생성). 웹 UI의 REVIEW.md 발췌 표시와
resume 원클릭 버튼(F1 후반부)도 2026-08-19에 구현 — 전체 완료.
